 ...
 * 
 * We plan to upgrade to to the latest version of the <a href="https://json-schema.org/">json-schema.org</a> incrementally.
 * Currently, this class represents what is essentially a fork of draft 03.  
 * 
 * @see <a
...
public class ObjectSchemaImpl implements ObjectSchema {
	 ...
	 * extensible recursive schemas.  A recursive schema is one that has a reference
	 ...
	 * @param type
	 */
	public ObjectSchemaImpl(TYPE type) {
		this.type = type;
	}
	...
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + (($recursiveAnchor == null) ? 0 : $recursiveAnchor.hashCode());
		result = prime * result + (($recursiveRef == null) ? 0 : $recursiveRef.hashCode());
		...
		result = prime * result + ((_transient == null) ? 0 : _transient.hashCode());
		result = prime * result + ((additionalItems == null) ? 0 : additionalItems.hashCode());
		...
		result = prime * result + ((disallow == null) ? 0 : disallow.hashCode());
		result = prime * result + ((divisibleBy == null) ? 0 : divisibleBy.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		ObjectSchemaImpl other = (ObjectSchemaImpl) obj;
		if ($recursiveAnchor == null) {
			if (other.$recursiveAnchor != null)
				return false;
		} else if (!$recursiveAnchor.equals(other.$recursiveAnchor))
			return false;
		if ($recursiveRef == null) {
			if (other.$recursiveRef != null)
				return false;
		} else if (!$recursiveRef.equals(other.$recursiveRef))
			return false;
		if (_default == null) {
	 ...
	 * @throws JSONObjectAdapterException
	 */
	public ObjectSchemaImpl(JSONObjectAdapter adapter)
			throws JSONObjectAdapterException {
	...
	public JSONObjectAdapter writeToJSONObject(JSONObjectAdapter copy)
		...
		if(this.$recursiveAnchor != null) {
			copy.put(JSON_RECURSIVE_ANCHOR, this.$recursiveAnchor.booleanValue());
		}
	...
	public static void recursivelyAddAllInterfaceProperties(
		...
		if (schema.getImplements() != null) {
			for (ObjectSchema implSchema : schema.getImplements()) {
				recursivelyAddAllInterfaceProperties(map, implSchema);
				if (implSchema.getProperties() != null) {
					map.putAll(implSchema.getProperties());
				}
			}
		}
	}
	...
	public static void recursivelyAddAllExtendsProperties(
		...
		if (schema.getExtends() != null) {
			recursivelyAddAllExtendsProperties(map, schema.getExtends());
			if (schema.getExtends().getProperties() != null) {
				map.putAll(schema.getExtends().getProperties());
			}
		}
	}
	...
	private static LinkedHashMap<String, ObjectSchema> createMapFromAdapter(
		...
		while (it.hasNext()) {
			...
			map.put(key, new ObjectSchemaImpl(adapter));
		}
		return map;
	}
	...
	public JSONObjectAdapter initializeFromJSONObject(JSONObjectAdapter adapter)
		...
		if (adapter.has(JSON_ITEMS)) {
			...
			this.items = new ObjectSchemaImpl(items);
		}
		...
		if (adapter.has(JSON_IMPLEMENTS)) {
			...
			for (int i = 0; i < array.length(); i++) {
				this._implements[i] = new ObjectSchemaImpl(array.getJSONObject(i));
			}
		}
		...
		if (adapter.has(JSON_EXTENDS)) {
			...
			this._extends = new ObjectSchemaImpl(ex);
		}
		...
		if (adapter.has(JSON_RECURSIVE_ANCHOR)) {
			this.$recursiveAnchor = adapter.getBoolean(JSON_RECURSIVE_ANCHOR);
		}
